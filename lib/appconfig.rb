...
module Configuration
  ...
  def self.hits_valuation(bookmarks, query)
    ...
    rgxs = query_split.map{|x| Regexp.new(x)} 
    # Case insensitive, any part of the string.
    rgxis = query_split.map{|x| Regexp.new(x,Regexp::IGNORECASE)}
    # Case insensitive, whole word match.
    rgxbs = query_split.map{|x| Regexp.new("\b#{x}\b",Regexp::IGNORECASE)}
    bookmarks.each {|entry|
      ...
      if entry[Bookmarks::RESPONSE] then
        ...
        entry[Bookmarks::SORT] += 2 if entry[Bookmarks::RESPONSE] && entry[Bookmarks::RESPONSE] =~ /^OK$/i
        entry[Bookmarks::SORT] += 1 if entry[Bookmarks::RESPONSE] && entry[Bookmarks::RESPONSE] =~ /^Found$/i
      end
    }
